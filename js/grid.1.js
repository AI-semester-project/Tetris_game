...
Grid.prototype.holes = function(){
    ...
    for(var c=1;c<this.columns;c++){
        let x_height =  this.columnHeight(c);
        for(var r=x_height;r<this.rows;r++){
            if(this.cells[r][c] == 0) {
                count++;
            }
        }
    }
    for(var c=this.columns-2;c>=0;c--){
        let x_height =  this.columnHeight(c);
        for(var r=x_height;r<this.row;r++){
            if(this.cells[r][c] == 0) {
                count++;
            }
        }
...
Grid.prototype.aggregateHeight = function(){
    var total = 0;
    for(var c = 0; c < this.columns; c++){
        total += this.columnHeight(c)*(Math.max(Math.abs(this.columns/2-c),1));
    }
    return total;
};
